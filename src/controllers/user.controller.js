...
const getUserChannelProfile = asyncHandler(async(req, res)=>{
    // to get user we can destrcut from params
    const {username} = req.params
    if(!username?.trim()){
        throw new ApiError(400, "username is missing")
    }
    const channel = await User.aggregate([
        // here we are matching the user
        {
            $match:{
                username:username?.toLowerCase()
            }
        ...
        {
            // adding fields in the original user object
            // that is subscribers count, channels subscribed
            $addFields:{
                subsribersCount:{
                    $size:"$subscribers"
                },
...
const getWatchHistory = asyncHandler(async(req, rs)=>{
    // here we get string 
